...
def csv_to_dataset(csv_path, history_points, offset, next_day_only=True):
    ...
    ohlcv_histories_normalised = np.array(
        [data_normalised[i:i + history_points].copy() for i in range(len(data_normalised) - history_points - offset)])

    if next_day_only:
        next_day_open_values_normalised = np.array([data_normalised[:, 0][i + history_points + offset].copy() for i in
                                                    range(len(data_normalised) - history_points - offset)])
        next_day_open_values_normalised = np.expand_dims(next_day_open_values_normalised, -1)

        next_day_open_values = np.array(
            [data[:, 0][i + history_points + offset].copy() for i in range(len(data) - history_points - offset)])
    ...
    for his in ohlcv_histories_normalised[:len(ohlcv_histories_normalised)]:
...
def csv_to_dataset_days(csv_path, history_points, days):
    data = pd.read_csv(csv_path)
    data = data.drop('date', axis=1)
    data = data.drop(0, axis=0)
    ...
    # using the last {history_points} open close high low volume data points, predict the next open value
    ohlcv_histories_normalised = np.array(
        [data_normalised[i:i + history_points].copy() for i in range(len(data_normalised) - history_points - days)])

    next_day_open_values_normalised = np.array(
        [[data_normalised[:, 0][i + history_points + j].copy() for j in range(days)] for i in
         range(len(data_normalised) - history_points - days)])

    next_day_open_values = np.array([[data[:, 0][i + history_points + j].copy() for j in range(data.shape[1])] for i in
                                     range(len(data) - history_points - days)])
